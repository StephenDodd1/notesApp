...
const openNote = (id) => {
  for(let i = 0; i < openButtons.length;  i++){
    ...
    if(i === id) {
      ...
      console.log('ran2')
      openButtons[id].classList.remove('hidden')
      closeButtons[id].classList.add('hidden')
...
for(let i = 0; i < openButtons.length;  i++){
  (function(idx){
    ...
    closeButtons[idx].addEventListener('click',function(event) {
      ...
      closeButtons[idx].classList.add('hidden')
      const currentNote = document.getElementById('note-details-'+i)
...
addNoteBtn.addEventListener('click', (e) => {
  ...
  saveButton.addEventListener('click', (e) => {
    ...
    const submittedNote = new Promise().resolve(() => 'success')
    submittedNote.resolve( (status) => {
      addNoteContainer.innerHTML = `<div>{add note - ${status}}</div>`
    })
  })
})